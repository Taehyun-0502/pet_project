...
export default function useCameraRecorder({ maxSec, onDone }) {
  ...
  useEffect(() => {
    ...
    /*
     * 처음부터 숏츠 규격(1080×1920)으로 받는다. 가로·세로를 **둘 다** 주는 것이 중요하다.
     *
     * 폰 센서는 4:3이라 9:16을 만들려면 좌우가 25% 잘린다. 이건 폰 기본 카메라의 '동영상 모드'가
     * 하는 것과 같은 양이고, 피할 방법이 없다 — 4:3을 9:16으로 만드는 길은 자르거나, 검은 띠를
     * 넣거나, 찌그러뜨리거나 셋뿐이다. 자르는 쪽을 택했다(2026-08-25 결정).
     *
     * 한때 "화면이 확대된 채 고정됐다"는 증상이 있었는데, 원인은 이 9:16 요청이 아니라 **두 번**
     * 잘린 것이었다: 여기서 한 번, 그리고 9:16이 아닌 뷰포트에 object-fit:cover가 걸려 또 한 번.
     * 지금은 미리보기 틀(.sc-frame)이 정확히 9:16이라 cover가 더 자르지 않는다.
     *
     * 가로를 비우고 높이만 주면 삼성 기기가 **1080×1080(정사각)** 모드를 고른다 — 그 포맷 목록에
     * 정사각이 있어서 높이 1080만 맞으면 완벽 일치로 뽑힌다. 실제로 갤럭시만 1:1로 나왔던 적이 있다.
     *
